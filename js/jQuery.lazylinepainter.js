(function($) {
    ...
    init: function(userOpts) {
      return this.each(function() {
        ...
        if (!data) {
          ...
          var longestDuration =
            options.delay + _getLongestDuration(options.paths);

          options.totalDuration = options.drawSequential
            ? totalDuration
            : longestDuration;
          _setupPaths(options);
  ...
  var _getOptions = function($this, userOpts) {
    ...
    var target = options.overrideKey
      ? options.overrideKey
  ...
  var _setupPaths = function(options) {
    ...
    for (var i = 0; i < options.paths.length; i++) {
      ...
      path.el.style.strokeDasharray = _getStrokeDashArray(
        path,
        options,
        path.length
      );
  ...
  var _getElapsedProgress = function(data, path) {
    ...
    if (
      data.progress > path.startProgress &&
      data.progress < path.startProgress + path.durationProgress
    ) {
      elapsedProgress =
        (data.progress - path.startProgress) / path.durationProgress;
  ...
  var _getAttributes = function(data, value) {
    return {
      ...
      "fill-opacity": 0,
      "stroke-opacity": !value.strokeOpacity
        ? data.strokeOpacity
        : value.strokeOpacity,
